...
from found_CLI_tools.attitude.constants import ROTATION_ORDER
...
class TransformTest(unittest.TestCase):
    ...
    def test_dcm_rotate_simple_inverse(self):
        ...
        actual = dcm.rotate(input, inverse=True).rotation.as_euler(
            ROTATION_ORDER, degrees=True
        )
    ...
    def test_dcm_rotate_general(self):
        for _ in range(10):
            ...
            expected_angles = list(
                (rotation_angles * input_angles).as_euler(ROTATION_ORDER, degrees=True)
            )
            actual_angles = list(
                rotation.rotate(input).rotation.as_euler(ROTATION_ORDER, degrees=True)
            )

            self.assertListEqual(expected_angles, actual_angles)

            expected_angles_inv = list(
                (rotation_angles.inv() * input_angles).as_euler(
                    ROTATION_ORDER, degrees=True
                )
            )
            actual_angles_inv = list(
                rotation.rotate(input, inverse=True).rotation.as_euler(
                    ROTATION_ORDER, degrees=True
                )
            )
