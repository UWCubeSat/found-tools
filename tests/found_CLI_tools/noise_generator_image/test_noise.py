    ...
    def test_all_functions_preserve_shape(self):
        ...
        for _ in range(TESTS):
            ...
            results = [
                ...
                add_salt_pepper_noise(
                    img, np.random.rand() * 0.02, np.random.rand() * 0.02
                ),
                apply_radial_distortion(
                    img, np.random.rand() * 0.1, np.random.rand() * 0.05, 0, 0
                ),
    ...
    def test_all_functions_maintain_valid_range(self):
        ...
        for _ in range(TESTS):
            ...
            results = [
                ...
                add_salt_pepper_noise(
                    img, np.random.rand() * 0.1, np.random.rand() * 0.1
                ),
                apply_radial_distortion(
                    img, np.random.rand() * 0.2, np.random.rand() * 0.1, 0, 0
                ),
                apply_motion_blur(img, int(np.random.randint(1, 10)) * 2 + 1),
