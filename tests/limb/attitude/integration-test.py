...
from limb.calibrate.transform import DCM, Attitude
...
from limb.calibrate.main import produce_attitudes, output_result
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_no_rotation(self):
        arguments = Arguments(
            DCM(), DCM(Rotation.from_euler(ROTATION_ORDER, [0, 0, 0], degrees=True))
        )
    ...
    def test_simple_rotation(self):
        arguments = Arguments(
            DCM(), DCM(Rotation.from_euler(ROTATION_ORDER, [135, 0, 0], degrees=True))
        )
    ...
    def test_rotation_general(self):
        ...
        for local, reference in tests:
            self.assertAttitudeOffsetGeneral(
                local, reference, arguments.calibration_attitude
            )
    ...
    def test_print_output(self):
        ...
        attitude_list = [
            ...
            *calibration_attitudes,
            calibration_attitudes[1],
            calibration_attitudes[0],
        ]
        ...
        ref_ori_matcher = re.compile(
            rf'^--reference-orientation "{number_list_matcher}"'
        )
        local_ori_matcher = re.compile(rf'^--local-orientation "{number_list_matcher}"')
        pair_matcher = (
            re.compile(rf"^{LOCAL_HEADER}{common_matcher}"),
            re.compile(rf"^{REFERENCE_HEADER}{common_matcher}"),
        )
        ...
        for line in lines:
            ...
            if match:
                self.assertDCMAlmostEqual(
                    attitude_list[i],
                    Attitude(*[float(num) for num in match.groups()]).to_dcm(),
                )
                i += 1
