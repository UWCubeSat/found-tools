...
class UniversalOpNavTester:
    ...
    def plot_radius_residuals_vs_range(
        ...
        """Plot radius residuals (pixels) vs true range (m) for all passes (Figure 7)."""
        indices = [i for i, img in enumerate(self.test_images) if img.target_body == target]
        if not indices:
            print(f"No data for {target.value}")
            return None
        ranges_m = np.array([self.test_images[i].range_m for i in indices])
        ...
        error_model = self.error_models[target](ranges_m[0] / 1000)
        sigma = error_model["radius_1sigma_px"]
        save_path = (self.output_dir / filename) if filename else None
        return plot_radius_residuals_vs_range(
            ranges_m=ranges_m,
            radius_residuals_px=radius_residuals,
            pass_ids=pass_ids,
            radius_1sigma_px=sigma,
            target_label=target.value.capitalize(),
            save_path=save_path,
        )
    ...
    def plot_centroid_residuals_in_illumination_frame(
        ...
        """Plot centroid residuals in illumination frame vs range (Figure 8)."""
        indices = [i for i, img in enumerate(self.test_images) if img.target_body == target]
        if not indices:
            print(f"No data for {target.value}")
            return None
        ...
        save_path = (self.output_dir / filename) if filename else None
        return plot_centroid_residuals_in_illumination_frame(
            ranges_m=ranges_m,
            centroid_x_px=centroid_x,
            centroid_y_px=centroid_y,
            pass_ids=pass_ids,
            centroid_1sigma_px=sigma,
            target_label=target.value.capitalize(),
            save_path=save_path,
        )
    ...
    def plot_earth_atmosphere_bias(self,
        ...
        ax.scatter(ranges_m, original_residuals, 
