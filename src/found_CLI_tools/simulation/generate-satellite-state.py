...
from found_CLI_tools.geometry.rotation import gramSchmidt, positionOnSurfaceEllipsoid

def generateUniformDirections():

def generateSatelliteState(shapeMatrix, earthPointDirection, distance,
                           cameraClass, edgeOffset,
                           numSatellitePositions, numSatelliteOrientations):
    ...
    smallestImageDimension = np.min([cameraClass.xResolution() * cameraClass.xPixelPitch(), 
                                    cameraClass.yResolution() * cameraClass.yPixelPitch()])
    imagePoint = np.array([cameraClass.focalLengt(), (smallestImageDimension - edgeOffset) / 2, 0])
    imageMaxEdgeAngle = np.arccos(np.dot(np.array([1, 0, 0]), imagePoint) / np.linalg.norm(imagePoint))
    ...
    rotationOffImagePixel = R.from_rotvec(imageEdgeAngle * rotatedTangentBasis[:, 1]).as_matrix()
    ...
    rotationAroundBoresight = R.from_rotvec(phi[:, None] * rotatedTangentBasis[:, 0]).as_matrix()
    ...
    satelliteOrientations = rotationAroundBoresight @ rotationOffImagePixel @ rotationFlipE0 @ rotatedTangentBasis
    ...
    satellitePositions = surfacePoint + distanceFromEdgePoint * rotatedTangentBasis[:, 0]
