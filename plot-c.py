...
# Centroid error in image plane (pixels): L2 distance between true and output centroid
df['abs_centroid_error'] = np.hypot(df['delta_x_centroid'], df['delta_y_centroid'])
...
accuracy_df = grouped.apply(lambda x: (x['abs_centroid_error'] < threshold).mean() * 100).reset_index(name='pct_below_thresh')
...
ax.set_title(f'Availability vs Range (km) (|Δc| < {threshold} px)', fontsize=FS_TITLE)
...
plt.savefig('multi_cam_connected_lines_c.png', bbox_inches='tight', pad_inches=0.12)
plt.close(fig)