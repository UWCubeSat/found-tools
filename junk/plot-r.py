...
df['abs_r_error'] = np.abs(df['out_r_apparent'] - df['true_r_apparent'])
...
accuracy_df = grouped.apply(lambda x: (x['abs_r_error'] < threshold).mean() * 100).reset_index(name='pct_below_thresh')
...
ax.set_title(f'Availability vs Range (km) (|Δr| < {threshold} px)', fontsize=FS_TITLE)
...
plt.savefig('multi_cam_connected_lines_r.png', bbox_inches='tight', pad_inches=0.12)
plt.close(fig)